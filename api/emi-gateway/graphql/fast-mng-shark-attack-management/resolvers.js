...
module.exports = {
    ...
    //// QUERY ///////
    Query: {
        FastMngSharkAttackListing(root, args, context) {
            return sendToBackEndHandler$(root, args, context, READ_ROLES, 'query', 'SharkAttack', 'FastMngSharkAttackListing').toPromise();
        },
        FastMngSharkAttack(root, args, context) {
            return sendToBackEndHandler$(root, args, context, READ_ROLES, 'query', 'SharkAttack', 'FastMngSharkAttack').toPromise();
        }
    },
    ...
    Mutation: {
        ...
        FastMngImportByCountrySharkAttack(root, args, context) {
            return sendToBackEndHandler$(root, args, context, WRITE_ROLES, 'mutation', 'SharkAttack', 'FastMngImportByCountrySharkAttack', 30000).toPromise();
        },
        ...
        FastMngDeleteSharkAttacks(root, args, context) {
            return sendToBackEndHandler$(root, args, context, WRITE_ROLES, 'mutation', 'SharkAttack', 'FastMngDeleteSharkAttacks').toPromise();
        },
    },
        ...
        FastMngSharkAttackModified: {
            subscribe: withFilter(
                (payload, variables, context, info) => {
                    ...
                    return pubsub.asyncIterator("FastMngSharkAttackModified");
                },
                (payload, variables, context, info) => {
                    return payload
                        ? (payload.FastMngSharkAttackModified.id === variables.id) || (variables.id === "ANY")
                        : false;
                }
            )
        }
...
const eventDescriptors = [
    {
        backendEventName: "FastMngSharkAttackModified",
        gqlSubscriptionName: "FastMngSharkAttackModified",
