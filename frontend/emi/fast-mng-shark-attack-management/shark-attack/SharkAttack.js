...
import {
    onFastMngSharkAttackModified,
    FastMngSharkAttack,
    FastMngCreateSharkAttack,
...
function SharkAttack(props) {
    ...
    const gqlSharkAttack = FastMngSharkAttack({ id: props.match.params.sharkAttackId });
    const [readSharkAttack, readSharkAttackResult] = useLazyQuery(gqlSharkAttack.query, { fetchPolicy: gqlSharkAttack.fetchPolicy })
    const [createSharkAttack, createSharkAttackResult] = useMutation(FastMngCreateSharkAttack({}).mutation);
    const [updateSharkAttack, updateSharkAttackResult] = useMutation(FastMngUpdateSharkAttack({}).mutation);
    const onSharkAttackModifiedResult = useSubscription(...onFastMngSharkAttackModified({ id: props.match.params.sharkAttackId }));
    ...
    */
    ...
    useEffect(() => {
        if (readSharkAttackResult.data)
            setSharkAttack(readSharkAttackResult.data.FastMngSharkAttack)
    }, [readSharkAttackResult])
    //Refresh SharkAttack state when the CREATE mutation resolves
    useEffect(() => {
        if (createSharkAttackResult.data && createSharkAttackResult.data.FastMngCreateSharkAttack) {
            setSharkAttack(createSharkAttackResult.data.FastMngCreateSharkAttack)
            props.history.push('/shark-attack-mng/shark-attacks/' + createSharkAttackResult.data.FastMngCreateSharkAttack.id + '/');
    ...
    useEffect(() => {
        if (updateSharkAttackResult.data) {
            setSharkAttack(updateSharkAttackResult.data.FastMngUpdateSharkAttack);
        }
    ...
    useEffect(() => {
        if (onSharkAttackModifiedResult.data) {
            setForm(onSharkAttackModifiedResult.data.FastMngSharkAttackModified);
